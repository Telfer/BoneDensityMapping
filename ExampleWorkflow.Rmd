...
```{r}
surface_colors <- color_mapping(surface_density, maxi = 1800, mini = 0)
```

Then, the colored mesh can be plotted.
```{r}
plot_mesh(surface_mesh, surface_colors, title = 'Bone surface', legend_maxi = 1800, legend_mini = 0)
```
...
```{r}
internal_colors <- color_mapping(internal_density, maxi = 1800, mini = 0)
```

A cross section of the bone can also be plotted to visualize internal density.
```{r}
plot_cross_section_bone(surface_mesh, surface_colors, IncludeSurface = FALSE, internal_fill, internal_colors, slice_axis = 'x', slice_val = 0.5, legend_maxi = 1800, legend_mini = 0)
```
...
```{r}
patients$SCAP001$surface_points <- surface_points_template(patients$SCAP001$mesh, patients$SCAP001$landmarks, 10000)  
```
