...
## --- Single bone analysis --- 
# If using for the first time, install and load the following package:
```{r}
install.packages(BoneDensityMapping)
library(BoneDensityMapping)
```

Next, import your data, including the bone mesh stl model, bone landmarks file, 
and the CT scan. 
```{r}
...
Next, given the surface mesh and landmark coordinates, the following function creates a remapped bone surface with a set number of surface points. 
```{r}
surface_points <- surface_points_template(surface_mesh, landmarks, 10000)  
```

The bone density values of these surface points can then be determined. The output, surface density, is a data frame of densities corresponding to  each point on the bone surface. 
```{r}
surface_density <- surface_normal_intersect(surface_mesh, surface_points, normal_dist = 3.0, CTscan)
```

To visualize the bone, we need to map the surface points and surface density values back to the stl mesh for graphing. To accomplish this, use the color_mesh function as shown:
```{r}
colored_mesh <- color_mesh(surface_mesh, surface_points, surface_density)
```

Then, the colored mesh can be plotted.
```{r}
plot_mesh(colored_mesh, density_color = NULL, 'Bone surface', ColorBar =TRUE)
```
...
for (i in 1:6) {
  ...
  mesh_path <- system.file("extdata", paste0(id, ".stl"), package = "BoneDensityMapping")
  landmark_path <- system.file("extdata", paste0(id, "_landmarks.fcsv"), package = "BoneDensityMapping")
  ct_path <- system.file("extdata", paste0(id, "_resampled.nrrd"), package = "BoneDensityMapping")
...
```{r}
patients$SCAP001$surface_points <- surface_points_template(patients$SCAP001$mesh, patients$SCAP001$landmarks, 1000)  
```
...
# Plot it
plot_mesh(diff_colored_mesh, title = "Young - Old Density Difference")
```
